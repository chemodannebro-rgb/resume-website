 ...
 *
 * Unlike the original version of this component, opentype.js is imported as a regular
 * npm dependency (not injected via a CDN <script> tag at runtime), and the font is
 * self-hosted under /public/fonts instead of being hotlinked from raw.githubusercontent.com,
 * so there is no runtime dependency on third-party infrastructure.
 *
 * If the font fails to load, the component renders the text as an ordinary <span> — it
...
export interface HandwritingTextProps {
  ...
  interval?: number;
  /** URL of a .ttf or .otf, served from /public. */
  fontUrl?: string;
...
export function HandwritingText({
  ...
  useEffect(() => {
    ...
    loadFont(fontUrl)
      ...
      .catch(() => { if (!cancelled) setFontFailed(true); });
  ...
  // If the font genuinely fails to load, the text is still readable.
  if (fontFailed) {
    return <span className={className}>{current}</span>;
  }
  // Still loading: render nothing rather than a flash of unstyled plain text
  // that would be replaced a moment later once the font resolves.
  if (!geom) {
    return null;
  }
    ...
    >
      {fill && (
        <path
          ...
          style={{
            opacity: drawn ? 1 : 0,
            transition: drawn && !reducedMotion
